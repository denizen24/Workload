...
const loadColor = (_load: number, taskType: string | null | undefined = null) => {
  ...
  if (isDuty) return "#facc15";
  if (isFeatureOrTech) return "#22c55e";
  if (isBug) return "#f97316";
  if (isTask) return "#4ade80";
  return "#4ade80";
};
...
export function GanttBoard({
  ...
  const range =
    sortedSprints.length > 0
      ? {
          start: startOfWeek(startSprint?.startDate ?? sortedSprints[0].startDate, {
            weekStartsOn: 1
          }),
  ...
  const buildTaskRanges = (assignee: Assignee) => {
    ...
    const taskMap = new Map<
      string,
      { start: Date; load: number; dayCount: number }
    >();
    ...
    dayMap.forEach((day) => {
      ...
      day.tasks.forEach((task) => {
        ...
        if (!existing) {
          taskMap.set(task, { start: date, load: share, dayCount: 1 });
          return;
        }
  ...
  const totalHeight = HEADER_HEIGHT + rowLayout.reduce((sum, row) => sum + row.height, 0) + 40;
  ...
  const endXForIndex = (index: number) => {
    const day = days[index];
    const workIndex = workDayIndex.get(format(day, "yyyy-MM-dd"));
    if (workIndex === undefined) return xForIndex(index);
  ...
  const computeEndIndex = (startIndex: number, workdayCount: number) => {
    ...
    while (index < days.length && remaining > 0) {
      ...
      const isWeekend = day.getDay() === 0 || day.getDay() === 6;
      const isHol = holidaysSet.has(format(day, "yyyy-MM-dd"));
      if (!isWeekend && !isHol) remaining -= 1;
    ...
    <div className="relative w-full overflow-auto rounded-2xl border border-slate-700/20 bg-white/70 p-4 shadow-sm dark:bg-slate-900/60">
      ...
      <svg width={totalWidth} height={totalHeight} className="min-w-full">
        ...
        <g>
          {sortedSprints.map((sprint) => {
            const startIndex = dayIndex.get(format(sprint.startDate, "yyyy-MM-dd")) ?? 0;
            const endIndex = dayIndex.get(format(sprint.endDate, "yyyy-MM-dd")) ?? 0;
            ...
            return (
              <g key={month.label}>
                <rect
                  ...
                  opacity={0.12}
                />
                <text x={x + 8} y={SPRINT_BAR_HEIGHT + 17} fontSize={12} fill="#94a3b8">
                  {month.label}
                </text>
              </g>
            );
          ...
          {rowOffsets.map((row) => {
            ...
            return (
              <g key={row.assignee.name}>
                <line x1={NAME_COLUMN_WIDTH} x2={totalWidth} y1={y} y2={y} stroke={rowStroke} />
                <line
                  ...
                  return (
                    ...
                    >
                      ...
                      <rect
                        ...
                        height={barHeight}
                        rx={4}
                        fill={loadColor(task.load, taskType)}
                        opacity={0.85}
                      ...
                      {taskTitle && (
                        <text x={x + 6} y={barY + 24} fontSize={8} fill="#475569">
                          {(() => {
      ...
      <div className="mt-4 flex flex-wrap items-center gap-4 border-t border-slate-500/20 pt-4 text-xs text-slate-600 dark:text-slate-400">
        ...
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-3 w-4 rounded" style={{ backgroundColor: "#16a34a" }} />
          FEATURE / TECH TASK
        </span>
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-3 w-4 rounded" style={{ backgroundColor: "#f97316" }} />
          BUG
        </span>
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-3 w-4 rounded" style={{ backgroundColor: "#4ade80" }} />
          TASK
        </span>
      </div>
    </div>
