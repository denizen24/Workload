...
type GanttBoardProps = {
  ...
  holidays?: string[];
  releaseDates?: string[];
  onTaskMove?: (payload: TaskMovePayload) => Promise<void> | void;
};
...
const HEADER_HEIGHT = SPRINT_BAR_HEIGHT + MONTH_BAR_HEIGHT + DAY_LABEL_HEIGHT + 8;
...
export function GanttBoard({
  ...
  const releaseDayMeta = useMemo(
    () => dayMeta.meta.filter((m) => releaseDatesSet.has(format(m.day, "yyyy-MM-dd"))),
  ...
  const workDayIndex = new Map(
    workDays.map((day, index) => [format(day, "yyyy-MM-dd"), index])
  );
  ...
  }, [data, customTasks, holidaysSet]);

  const taskIndex = useMemo(() => {
    const map = new Map<
      string,
      { task: { id: string; start: Date; widthDays: number }; assignee: string }
    >();
    rowLayout.forEach((row) => {
      row.tasks.forEach((task) => {
        map.set(`${row.assignee.name}::${task.id}`, {
          task,
  ...
  useEffect(() => {
    ...
    const handleUp = () => {
      const activeTaskId = dragState.taskId;
      setDragState({ taskId: null, startX: 0, startOffset: 0 });
      if (!activeTaskId) return;
      const offset = dragOffsets[activeTaskId] ?? 0;
      if (!offset || !onTaskMove) {
        setDragOffsets((prev) => {
          const { [activeTaskId]: _, ...rest } = prev;
          return rest;
        });
        return;
      }

      const meta = taskIndex.get(activeTaskId);
      if (!meta || meta.task.id.startsWith("custom-")) {
        setDragOffsets((prev) => {
          const { [activeTaskId]: _, ...rest } = prev;
          return rest;
        });
      ...
      const startKey = format(meta.task.start, "yyyy-MM-dd");
      const startWorkIndex = workDayIndex.get(startKey);
      if (startWorkIndex === undefined) {
        setDragOffsets((prev) => {
          const { [activeTaskId]: _, ...rest } = prev;
          return rest;
        });
      ...
      const adjustedIndex = clamp(startWorkIndex + offset, 0, workDays.length - 1);
      const nextStart = workDays[adjustedIndex];
      const payload: TaskMovePayload = {
        issueId: meta.task.id,
        assignee: meta.assignee,
        newStartDate: format(nextStart, "yyyy-MM-dd"),
        durationDays: meta.task.widthDays
      };

      const result = onTaskMove(payload);
  ...
  }, [dragState, dragOffsets, onTaskMove, taskIndex, workDayIndex, workDays]);
    ...
    <div className="relative w-full overflow-auto rounded-2xl border border-slate-700/20 bg-white/70 p-4 shadow-sm dark:bg-slate-900/60">
            ...
            return (
              <g key={month.label}>
                <rect
                  ...
                  opacity={0.12}
                />
                <text x={x + 8} y={SPRINT_BAR_HEIGHT + 2 + 15} fontSize={12} fill="#94a3b8">
                  {month.label}
                </text>
              </g>
            );
            ...
            return (
              <g key={row.assignee.name}>
                ...
                {row.tasks.map((task, taskIndex) => {
                  ...
                  const adjustedStartIndex = clamp(startIndex + offset, 0, days.length - 1);
                  ...
                  return (
                    <g
                      ...
                      style={{ cursor: isCustom ? "default" : "grab" }}
                    >
                      ...
                      {taskTitle && (
                        <text x={x + 6} y={barY + 24} fontSize={8} fill="#475569" width={width - 12}>
                          {(() => {
                            const maxChars = Math.floor((width - 12) / 5);
                            return taskTitle.length > maxChars
                              ? `${taskTitle.substring(0, Math.max(0, maxChars - 3))}...`
                              : taskTitle;
                          })()}
                        </text>
                      )}
      ...
      <div className="mt-4 flex flex-wrap items-center gap-4 border-t border-slate-500/20 pt-4 text-xs text-slate-600 dark:text-slate-400">
        ...
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-3 w-4 rounded" style={{ backgroundColor: "#16a34a" }} aria-hidden />
          FEATURE / TECH TASK
        </span>
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-3 w-4 rounded" style={{ backgroundColor: "#f97316" }} aria-hidden />
          BUG
        </span>
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-3 w-4 rounded" style={{ backgroundColor: "#4ade80" }} aria-hidden />
          TASK
        </span>
      </div>
    </div>
